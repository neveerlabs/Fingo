function initCommon(page) {
    const bottomNavItems = [
        { id: 'home', href: '../', label: 'Home', svg: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12l9-9 9 9"/><path d="M5 10v10h14V10"/></svg>' },
        { id: 'insight', href: 'insight.html', label: 'Insight', svg: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>' },
        { id: 'wallet', href: 'wallet.html', label: 'Wallet', svg: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="14" rx="2"/><path d="M2 10h20"/><path d="M16 15h2"/></svg>' },
        { id: 'profile', href: 'profile.html', label: 'Profile', svg: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 4-6 8-6s8 2 8 6"/></svg>' }
    ];

    const navContainer = document.querySelector('.bottom-nav');
    if (navContainer) {
        let navHTML = '';
        navHTML += `<a href="${bottomNavItems[0].href}" class="nav-item ${page === 'home' ? 'active' : ''}">${bottomNavItems[0].svg}<span>${bottomNavItems[0].label}</span></a>`;
        navHTML += `<a href="${bottomNavItems[1].href}" class="nav-item ${page === 'insight' ? 'active' : ''}">${bottomNavItems[1].svg}<span>${bottomNavItems[1].label}</span></a>`;
        navHTML += `<div class="nav-item plus" id="plusNavItem"><div class="plus-circle" id="plusCircle"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div><div class="fab-menu" id="fabMenu"><div class="action-item" id="incomeAction"><div class="circle income"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 11 12 16 7 11"/><line x1="12" y1="16" x2="12" y2="4"/></svg></div><span class="action-label">Income</span></div><div class="action-item" id="expenseAction"><div class="circle expense"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="7 13 12 8 17 13"/><line x1="12" y1="8" x2="12" y2="20"/></svg></div><span class="action-label">Expense</span></div></div></div>`;
        navHTML += `<a href="${bottomNavItems[2].href}" class="nav-item ${page === 'wallet' ? 'active' : ''}">${bottomNavItems[2].svg}<span>${bottomNavItems[2].label}</span></a>`;
        navHTML += `<a href="${bottomNavItems[3].href}" class="nav-item ${page === 'profile' ? 'active' : ''}">${bottomNavItems[3].svg}<span>${bottomNavItems[3].label}</span></a>`;
    ...
    if (modalContent) {
        modalContent.innerHTML = `
            ...
            <a href="wallet.html" class="modal-item"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="14" rx="2"/><path d="M2 10h20"/><path d="M16 15h2"/></svg><span>Wallet</span></a>
    ...
    if (usernameOverlay) {
        ...
        usernameForm.addEventListener('submit', (e) => {
            ...
            if (/^[a-z0-9]+$/.test(username)) {
                ...
                document.getElementById('username-error').textContent = 'Only lowercase letters and numbers are allowed.';
            }
        });
        const input = document.getElementById('username-input');
    ...
    function updatePreview() {
        ...
        categoryPreview.innerHTML = `<div class="category-preview-box" style="border-color:${selectedColor};"><span class="category-icon" style="color:${selectedColor}">${getIconSvg(selectedIcon)}</span><div class="category-info"><span class="category-name">${name}</span><span class="category-group">${group}</span></div></div>`;
    }
    ...
    allCategoriesLink.addEventListener('click', () => {
        if (categoriesAllList.classList.contains('hidden')) showAllListView();
        else showChipsView();
    });
    ...
    saveCategoryBtn.addEventListener('click', async () => {
        ...
        if (editingCategoryId) categories = categories.map(c => c.id === editingCategoryId ? newCategory : c);
